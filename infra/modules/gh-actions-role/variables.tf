...
variable "default_conditions" {
  ...
  validation {
    condition     = length(setsubtract(var.default_conditions, ["allow_main", "allow_master", "allow_environment", "deny_pull_request", "allow_all"])) == 0
  ...
  validation {
    ...
    error_message = "At least one of the following configuration needs to be set: 'allow_main', 'allow_master', 'allow_environment', 'deny_pull_request' and 'allow_all'."
  }
}
